  ...
  <section id="container">
   ...
   <!--sidebar start-->
    ...
    <section id="main-content">
      ...
      <h3>Stock List</h3>  
       <!--     <div class="showback">            
          <a href="add_stock.php"><button type="button" class="btn btn-theme"> Add New Stock <i class="fa fa-edit"></i></button></a>    
            
            </div> --> 
        <div class="row mt">
          <div class="col-lg-12">
            <div class="content-panel">
               <div class="col-lg-6"><h4>Stock Details</h4></div>
...
$sql = "SELECT a.*,d.category,b.company_name as c_name,c.weight as weight1,c.id as s_id,c.sn,c.size,c.balance_qty FROM  stock a,product_company b,stock_weight c,category d where d.id=a.category and a.company_name=b.id and c.stock_id=a.id";
...
?>
     ...
     while($row = $result->fetch_assoc()) {
         ?>
     <tr>
   					   	<td data-title="ID" style="display:none"><?php echo $row["id"];  ?></td>
   						<td data-title="ID"><?php echo $count; ?></td>
                     	<td data-title="company Name"><?php echo $row["c_name"];  ?></td>
                    	<td data-title="produc Name"><?php echo $row["product_name"];  ?></td>
               		
               		    <td  data-title="Category"><?php echo $row["category"];  ?> </td>
               		    <td data-title="sub_product Name" style="display:none"><?php echo $row["sn"];  ?></td>
               		    <td data-title="Weight "><?php echo $row["weight1"];  ?><?php echo $row["size"];  ?> </td>
               		    <td data-title="balance_qty Name"><?php echo $row["balance_qty"];  ?></td>
               			
                     	
                                	
	<td data-title="Action">
               
                 <a href='view_available_stock_details.php?id=<?php echo $row["id"]; ?>&sn=<?php echo $row["sn"]; ?>&weight=<?php echo $row["weight1"];?>&size=<?php echo $row["size"]; ?>'> <button class="btn btn-primary btn-xs">View Stock</button></a>
                 <a href='view_available_stock_details_show.php?&id=<?php echo $row["id"]; ?>&sn=<?php echo $row["sn"]; ?>&weight=<?php echo $row["weight1"];?>&size=<?php echo $row["size"]; ?>'> <button class="btn btn-primary btn-xs">In/Out Stock</button></a>
